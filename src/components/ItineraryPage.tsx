...
const ItineraryList = ({ itineraries, isLoading, onView, onEdit, onDuplicate, onDelete, onCreateNew }: ItineraryListProps) => {
  if (isLoading) {
    return (
      <div className="max-w-md mx-auto px-4 py-12 flex items-center justify-center">
        ...
        return (
          ...
          >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {it.vibes?.slice(0, 3).map(v => (
                      <span key={v} className="text-[10px] font-medium text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">
                        {v}
                      </span>
                    ))}
                    {totalActivities > 0 && (
            ...
            <div className="flex items-center border-t border-border/50 divide-x divide-border/50">
              <button
                onClick={() => onEdit(it)}
                className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors"
              >
