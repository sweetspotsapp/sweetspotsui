...
const AISummaryCard = ({ summary, searchQuery, location }: AISummaryCardProps) => {
  ...
  const getBulletPoints = (): string[] => {
    ...
    let cleaned = summary;if (location && location !== "nearby") {cleaned = cleaned.replace(/\s*nearby\.?/gi, '.');cleaned = cleaned.replace(/\s*within \d+(\.\d+)?\s*(km|miles?)\.?/gi, '.');cleaned = cleaned.replace(/\.\./g, '.').trim();}return cleaned.split(/(?<=\.)\s+/).map((s) => s.trim()).filter((s) => s.length > 5).slice(0, 3);
  };
  ...
  return (
    <div className="mx-4 mb-6 rounded-2xl bg-gradient-to-br from-amber-50/80 to-orange-50/60 dark:from-amber-950/30 dark:to-orange-950/20">
      <button
        ...
        className="w-full flex items-center gap-3 p-4 text-left rounded-2xl bg-amber-100/60 dark:bg-amber-900/40 hover:bg-amber-100/80 dark:hover:bg-amber-900/50 active:bg-amber-200/60 dark:active:bg-amber-900/60 transition-colors py-[8px]">
