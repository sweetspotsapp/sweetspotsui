...
const matchesFilter = (place: ExtendedMockPlace, filterId: string): boolean => {
  ...
  // Normalize both sides (hyphens ↔ underscores) so tags match regardless of format
  const normalize = (s: string) => s.toLowerCase().replace(/-/g, '_');
  const normalizedExpected = normalize(expectedTag);
  return tags.some(t => normalize(t) === normalizedExpected);
};
